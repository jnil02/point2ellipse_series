...
Expression d_phi(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	...
	for (int r = 0; r <= k - 1; ++r)
		for (int m = 0; m <= k - 1 - r; ++m)
			for (int q = 0; q <= r / 2; ++q)
				for (int p = 0; p <= m / 2; ++p)
					for (int t = l - n - k + m + r - p - q; t <= std::min({l - k, r - q, l - n - k + m + r - q}); ++t)
						d += rf_half(k, r - q) * (powm1(n - l - k) * (1L << (r - 2 * q)))
								/ (Expression(factorial(q)) * factorial(r - 2 * q) * (m + 1 + r))
								* binomial(Integer(r - q), t)
								* binomial(Integer(k - 1), m + r)
								* binomial(Integer(m + 1), 2 * p + 1)
								* binomial_rational(add(rational(k, 2), integer(r - q + l - k - t - 1)), l - k - t)
								* binomial(Integer(p), n - (m + r - q + l - k - t - p));

	cache.insert(d, (uint) n, (uint) k, (uint) l);
	return d;
}

inline Expression d_phi2(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
	...
	for (int r = 0; r <= k - 1; ++r)
		for (int m = 0; m <= k - 1 - r; ++m)
			for (int q = 0; q <= r / 2; ++q)
				for (int p = 0; p <= m / 2; ++p)
					for (int t = l - n - k + m + r - p - q; t <= std::min(l - k, r - q); ++t)
							d += rf_half(k, r - q) * (powm1(n - l - k) * (1L << (r - 2 * q)))
									/ (Expression(factorial(q)) * factorial(r - 2 * q) * (m + 1 + r))
									* binomial(Integer(r - q), t)
									* binomial(Integer(k - 1), m + r)
									* binomial(Integer(m + 1), 2 * p + 1)
									* binomial_rational(add(rational(k, 2), integer(r - q + l - k - t - 1)), l - k - t)
									* binomial_rational(add(rational(1, 2), integer(p)), n - (m + r - q + l - k - t - p));

	cache.insert(d, (uint) n, (uint) k, (uint) l);
	return d;
}

Expression c_phi(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
			...
			for (int p = 0; p <= m / 2; ++p)
				for (int q = 0; q <= r / 2; ++q)
					for (int t = 0; t <= std::min(l - k, r - q); ++t) {
						...
						Expression h(0);
						for (int i = std::max(0, p - n + 1); i <= std::min(p, p - n + 1 + w); ++i) {
							const int j = w + p - i + 1 - n;
							h += Expression(binomial(SymEngine::Integer(2 * p + 1), i))
						...
						d += h * rf_half(k, r - q) * powm1(l - k)
								/ (Expression(SymEngine::factorial(q)) * SymEngine::factorial(r - 2 * q) * Expression(m + 1 + r) * (1u << (2 * (m + l - k - t) + r + 1)))
								* binomial(Integer(r - q), t)
								* binomial(Integer(k - 1), m + r)
								* binomial(Integer(m + 1), 2 * p + 1)
								* binomial_rational(add(rational(k, 2), integer(r - q + l - k - t - 1)), l - k - t);
...
inline Expression
d_phi_pow_polynomial(int n, int k, int i) {
	assert(n >= 0 && k >= 0 && i >= 0);
	static auto cache = UintsCache<Expression>();
...
inline Expression d_phi_pow(int n, int k, int l, int i) {
	...
	cache.insert(d, (uint) n, (uint) k, (uint) l, (uint) i);
	return d;
}

inline Expression d_sin(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
	...
	for (int i = 1; i <= i_max; ++i) {
		...
		for (int j = std::max(0, ceil_i_2 - l + n); j <= std::min(ceil_i_2, n - floor_i_2); ++j)
			d += Expression(powm1(floor_i_2 + j))
					* binomial(Integer(ceil_i_2), j)
					* d_phi_pow(n - floor_i_2 - j, k, l, i)
...
inline Expression d_sin_pow_polynomial(int n, int k, int i) {
	assert(n >= 0 && k >= 0 && i >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) i))
		return *ret;
...
inline Expression d_sin_pow(int n, int k, int l, int i) {
	assert(n >= 0 && k >= 0 && l >= 0 && i >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l, (uint) i))
		return *ret;
...
inline Expression c_sin(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
...
inline Expression d_cos(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
	...
	for (int i = 1; i <= i_max; ++i) {
		const int floor_i_2 = i / 2;
		const int ceil_i_2 = (i + 1) / 2;
		for (int j = std::max(0, floor_i_2 - l + n); j <= std::min(floor_i_2, n - ceil_i_2); ++j)
			d += Expression(powm1(ceil_i_2 + j))
					* binomial(Integer(floor_i_2), j)
					* d_phi_pow(n - ceil_i_2 - j, k, l, i)
					/ SymEngine::factorial(i);
	}
...
inline Expression c_cos(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
...
inline Expression d_N_nkl(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
	...
	for (int i = 1; i <= std::min(n, l); ++i)
		for (int j = 0; j <= std::min(2 * i, k); ++j)
			d += binomial_rational(rational(1, 2), i)
					...
					* Expression(powm1(i))
					* d_sin_pow(n - i, k, l - i, j);

	cache.insert(d, (uint) n, (uint) k, (uint) l);
	return d;
}

inline Expression bp_nkl(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
		...
		for (int j = 0; j <= std::min(i, n - i); ++j)
			d += Expression(powm1(i + j))
					* binomial(Integer(i), j)
					/ factorial(2 * i)
					* d_phi_pow(n - i - j, k, l, 2 * i);

	cache.insert(d, (uint) n, (uint) k, (uint) l);
	return d;
}

inline Expression d_h(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
	...
	if (k == 0) {
		...
		d = bp_nkl(n, k + 1, l) - d_N_nkl(n, k, l);
	}
	cache.insert(d, (uint) n, (uint) k, (uint) l);
	return d;
}

inline Expression c_h(int n, int k, int l) {
	assert(n >= 0 && k >= 0 && l >= 0);
	static auto cache = UintsCache<Expression>();
	if (auto *ret = cache.get((uint) n, (uint) k, (uint) l))
		return *ret;
